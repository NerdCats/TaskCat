        ...
        public async Task<AssetJobCountModel> FindEligibleAssetJobCounts(string AssetID)
        {
            ...
            if (jobsAssignedToAsset != null)
            {
                ...
                if (jList_Pick_Up != null)
                {
                    ...
                    ajcm.Pick_Up.Completed = jList_Pick_Up.Count(y => y.Tasks.Exists(x => x.State == JobTaskState.COMPLETED));// total jobs COMPLETED
                    ajcm.Pick_Up.Failed = jList_Pick_Up.Count(y => y.Tasks.Exists(x => x.State == JobTaskState.FAILED));// total jobs FAILED
                    ajcm.Pick_Up.Attempted = ajcm.Pick_Up.Completed + ajcm.Pick_Up.Failed > ajcm.Pick_Up.Assigned ? ((ajcm.Pick_Up.Completed > ajcm.Pick_Up.Failed) ? ajcm.Pick_Up.Completed : ajcm.Pick_Up.Failed) : ajcm.Pick_Up.Completed + ajcm.Pick_Up.Failed; // sum of all jobs ATTEMPTED 
                }
                ...
                if (jList_Delivery != null)
                {
                    ...
                    ajcm.Delivery.Completed = jList_Delivery.Count(y => y.Tasks.Exists(x => x.State == JobTaskState.COMPLETED));
                    ajcm.Delivery.Failed = jList_Delivery.Count(y => y.Tasks.Exists(x => x.State == JobTaskState.FAILED));
                    ajcm.Delivery.Attempted = ajcm.Delivery.Completed + ajcm.Delivery.Failed > ajcm.Delivery.Assigned ? ((ajcm.Delivery.Completed > ajcm.Delivery.Failed) ? ajcm.Delivery.Completed : ajcm.Delivery.Failed) : ajcm.Delivery.Completed + ajcm.Delivery.Failed; // sum of all jobs ATTEMPTED 
                }
